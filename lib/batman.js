  ...
  Batman.Model = (function() {
    ...
    Model.prototype.save = function(callback) {
      ...
      return this.validate(__bind(function(isValid, errors) {
        var associations, belongsTo, creating;
        if (!isValid) {
          if (typeof callback === "function") {
            callback(errors);
          }
        ...
        if (associations = this.constructor.associations) {
          if (belongsTo = associations.belongsTo) {
            belongsTo.forEach(__bind(function(key) {
              var model;
              if ((model = this.get(key)) && model.state() !== "saved") {
                return model.save(__bind(function(err, record) {
                  if (err) {
                    throw err;
                  }
                  return this.set("" + key + "_id", record.id);
                }, this));
              } else if (model) {
                return this.set("" + key + "_id", model.id);
              }
            }, this));
          }
        }
